        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (int i = 0; i < insts.Count; i++)
            {
                ...
                if (!match && i > 1 && insts[i - 1].IsStloc() && insts[i].IsLdloc()) //If it's the pattern we are matching.. (in this case the bit of IL after the variable we want is assigned)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(DangerWatcher_CalculateDangerRating), "_putDangerRatingPerMap")); //Call our method using the stuff on the stack, leaving it how it was before we did anything.
                }
            }
        }
    }
