...
namespace BiomesCore.Patches
{
	...
	internal static class WildPlantSpawner_GetBaseDesiredPlantsCountAt
	{
		...
		public static void UpdateCommonalityAt(IntVec3 c, Map map, TerrainDef terrain = null, float? baseDesiredPlantCountAt = null)
		{
			...
			foreach (ThingDef plant in map.Biome.AllWildPlants)
			{
				...
				if (plantControl != null)
				{
					if (plantControl.terrainTags != null)
					{
						foreach (string tag in plantControl.terrainTags)
						{
							...
							commonalitySum[tag] += map.Biome.CommonalityOfPlant(plant);
						}
					}
					else
						commonalitySum["Soil"] += map.Biome.CommonalityOfPlant(plant) * 2f;
				}
				else
					commonalitySum["Soil"] += map.Biome.CommonalityOfPlant(plant) * 2f;
			}
