		...
		public override void Regenerate()
		{
			...
			foreach (IntVec3 item in cellRect)
			{
				if (ShouldDrawPropsBelow(item, terrainGrid))
				{
					...
					subMesh.verts.Add(new Vector3((float) item.x, y, (float) item.z));
					subMesh.verts.Add(new Vector3((float) item.x + 1f, y, (float) item.z));
					subMesh.verts.Add(new Vector3((float) item.x + 1f, y, (float) item.z - 0.9f));
