...
{
	public class TerrainCompProperties_BurnItems : TerrainCompProperties
	{
		public TerrainCompProperties_BurnItems()
		{
			compClass = typeof(TerrainComp_BurnItems);
		}
	}
	...
	public class TerrainComp_BurnItems : TerrainComp
	{
		...
		public TerrainCompProperties_BurnItems Props
		{
			get { return (TerrainCompProperties_BurnItems) props; }
		}
			...
			for (var i = allThings.Count - 1; i >= 0; i--)
			{
				...
				if (t.def.BaseFlammability > 0)
				{
					...
					if (t is Pawn pawn)
					{
						...
						if (pawn.health.hediffSet.GetNotMissingParts(BodyPartHeight.Bottom, BodyPartDepth.Outside)
						    .Where(x => x.coverageAbs > 0).TryRandomElement(out var part))
						{
